...
function updateMetaData() {
  ...
  updateStatus(track, 'Playing: '+track.title);
...
const actionHandlers = [
  ...
  [
    ...
    async function() {
      ...
      updateStatus(allMeta[index], 'playing: '+allMeta[index].title)
    }
  ],
  [
    'pause',
    () => {
      ...
      // update our status element
      updateStatus(allMeta[index], 'paused: '+allMeta[index].title);
    }
  ],
  [
    'previoustrack',
    () => {
      ...
      let statusMess = 'playing previous: '+allMeta[index].title;
      if (index === 0) {
        statusMess = 'No previous track, first track playing: '+allMeta[index].title;
      } else {
        index = index - 1;
      }
      // play
      playAudio();
      // update our status element
      updateStatus(allMeta[index], 'Skipback <|  '+statusMess);
    }
  ],
  ['nexttrack',
    () => {
      // if it's the last track seek to the end of the track
      let statusMess = 'Nexted playing: '+allMeta[index].title;
      if (index === (allMeta.length-1)) {
        ...
        // play
        playAudio();
        updateStatus(allMeta[index], 'skipforward  >|  '+statusMess);
      }
      // update our status element
  ...
  [
    ...
    () => {
      ...
      index = 0;
      // update our status element
      updateStatus(allMeta[index], 'stop  |  Playlist has stopped.');
    }
  ],
  [
    'seekbackward',
...
audioEl.addEventListener('play', function() {
  if (first) {
    const message = isSupported ? "playing: "+allMeta[index].title : "MediaSession not supported";
